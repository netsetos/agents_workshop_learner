...
| 2 | [demo_01_stream_events_and_empty_pool.py](demo_01_stream_events_and_empty_pool.py) | Time the event sequence and inspect citations plus the one-token empty-pool stream. |
| 3 | [demo_02_guard_and_reranker_failure.py](demo_02_guard_and_reranker_failure.py) | Read guard configuration and force degraded streaming on a no-traffic candidate. |
| 4 | [demo_03_stream_usage_by_surface.py](demo_03_stream_usage_by_surface.py) | Read stream/query usage by surface and explain which costs the records include. |
...
### demo_01_stream_events_and_empty_pool.py
...
**`step_02_the_empty_pool_as_one_token_then_the_strea(session)` — What a stream's citations are, and the streams of one token / Do it: the empty pool as one token, then the stream rows**
...
### demo_02_guard_and_reranker_failure.py
...
**`step_02_the_ranker_silent_for_one_revision_the_str(session)` — A failure forced on a candidate: the ranker silent, the stream still served, the line in the log / Do it: the ranker silent for one revision, the stream read, the line read, the undo**
...
### demo_03_stream_usage_by_surface.py
...
[Reading guide](GUIDE.md) retains explanatory prose and UI instructions from the [main HTML](https://github.com/netsetos/agents_workshop/blob/main/lessons/06-generation/6.3-streaming/Netsetos_GCP_Capstone_6.3_Streaming_WIX.html). All 36 original windows are accounted for in `lesson_map.json`: executable steps, shared setup, or read-only examples. Reviewed source: `c6e58b623f6b0a931efccb868a7e7dc1ba3f7f54`.
